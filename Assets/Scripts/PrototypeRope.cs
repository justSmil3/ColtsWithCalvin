...
//                }
//                if (i == 0)
//                    SpringOne = this.GetComponent<Rigidbody>().velocity * Time.deltaTime / accuracy;
//                else if (i == virtualSegments.Length - 1)
//                    SpringTwo = Target.GetComponent<Rigidbody>().velocity * Time.deltaTime / accuracy;



//                float ks = stiffness + gravitationMultiplyer;
//                Vector3 fSpringOne = Vector3.zero, fSpringTwo = Vector3.zero;
//                float s1d = (SpringOne.magnitude - segmentDistance);
//                float s2d = (SpringTwo.magnitude - segmentDistance);
//                fSpringOne = -ks * (Mathf.Abs(s1d)/s1d + s1d) * SpringOne.normalized;
//                fSpringTwo = -ks * (Mathf.Abs(s2d) / s2d + s2d) * SpringTwo.normalized;
//                Debug.DrawRay(data.position, fSpringTwo / (ks), Color.white);
//                Debug.DrawRay(data.position, -fSpringOne / (ks), Color.grey);
...
//                Vector3 force = fSpringTwo + fGravity * Time.deltaTime - fSpringOne;

//                Vector3 avelocity = velocity + force * Time.deltaTime; // + data.veloChange * Time.deltaTime;
//                data.velocity = avelocity / accuracy;

//                data.veloAddition = (fSpringTwo - fSpringOne) * Time.deltaTime;
...
//                    // take care of that
//                    Vector3 v = data.veloAddition / accuracy;
//                    Instanciator.GetComponent<Rigidbody>().velocity += v;
//                    Debug.DrawRay(data.position, v, Color.black);
